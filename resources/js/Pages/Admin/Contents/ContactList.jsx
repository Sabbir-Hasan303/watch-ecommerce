...
export default function ContactList({ contacts }) {
    ...
    const filteredContacts = normalized.filter(contact => {
        const matchesSearch =
        ...
        const statusMap = ['all', 'new', 'replied']
    ...
    const newCount = normalized.filter(c => (c.status || 'new') === 'new').length
    const repliedCount = normalized.filter(c => c.status === 'replied').length
                        ...
                        <Card className='p-6'>
                            <div className='flex items-center justify-between'>
                                <div>
                                    ...
                                    <p className='text-3xl font-bold text-text-primary'>{normalized.length}</p>
                                </div>
                                        ...
                                        <TableRow
                                            ...
                                            <TableCell className='table-body-cell dark:table-body-cell'>
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                    <Calendar size={16} />
                                                    <Typography variant='body2'>{contact.date ? new Date(contact.date).toLocaleDateString() : '-'}</Typography>
                                                </Box>
                                            </TableCell>
                                            <TableCell className='table-body-cell dark:table-body-cell'>
                                                <Chip
                                                    label={(contact.status || 'new').charAt(0).toUpperCase() + (contact.status || 'new').slice(1)}
                                                    size='small'
                                                    sx={{
                                                        bgcolor:
                                                            (contact.status || 'new') === 'new'
                                                                ? 'rgba(249, 115, 22, 0.1)'
                                                                : 'rgba(34, 197, 94, 0.1)',
                                                        color: (contact.status || 'new') === 'new' ? '#F97316' : '#22C55E',
                                                        border: 'none',
                                                        '&:hover': {
                                                            bgcolor:
                                                                (contact.status || 'new') === 'new'
                                        ...
                                        sx={{
                                            bgcolor:
                                                selectedContact.status === 'new'
                                                    ? 'rgba(249, 115, 22, 0.1)'
                                                    : 'rgba(34, 197, 94, 0.1)',
                                            color: selectedContact.status === 'new' ? '#F97316' : '#22C55E',
                                            border: 'none'
                                        }}
                                    />
                                </Box>
                ...
                <DialogActions sx={{ p: 3, gap: 1 }}>
                    ...
                    <Button
                        ...
                        onClick={() => {
                            ...
                            router.post(route('admin.contents.contact-list.mark-as-replied'), { id: selectedContact.id }, {
                                preserveScroll: true,
