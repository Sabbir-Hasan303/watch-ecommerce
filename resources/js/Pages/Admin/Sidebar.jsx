...
export default function Sidebar({ collapsed, onToggle, user }) {
    ...
    const managementItems = [
        ...
        },
        {
            ...
            subItems: [
                ...
                { label: 'Trending Products', href: route('admin.products.featured-products') }
            ]
        },
        {
            icon: ShoppingBag,
    ...
    const renderMenuItem = (item, index) => {
        ...
        const buttonClasses = cn(
            'w-full flex items-center px-3 py-2.5 rounded-xl transition-all duration-300 group relative overflow-hidden',
            !collapsed && 'gap-3',
            isActive || hasActiveChildItem
                ? 'text-[#00a76f] dark:text-[#5be49b] bg-[rgba(0,167,111,0.08)] dark:bg-[rgba(0,167,111,0.15)]'
                : 'text-text-secondary hover:text-text-primary hover:!bg-[#F3F4F6] dark:hover:!bg-[#1D242B] hover:scale-[1.02]',
        ...
        return (
            <div key={index} className='relative'>
                ...
                {item.href && !hasSubItems ? (
                    ...
                    <Ripple onClick={() => toggleExpanded(item.label)} className={buttonClasses} bgColor='bg-[#00a76f]/40 dark:bg-[#5be49b]/40'>
                        <div className={cn('relative z-10 flex items-center', !collapsed && 'gap-3 w-full')}>{buttonContent}</div>
                    </Ripple>
                ) : (
        ...
        <>
            ...
            <aside
                ...
                <button
                    type='button'
                    onClick={onToggle}
                    className='hidden lg:flex items-center justify-center w-7 h-7 rounded-full border border-gray-600/40 text-gray-400 hover:text-text-primary hover:dark:text-white hover:border-gray-400 bg-background absolute top-[38px] z-40 -translate-y-1/2'
                    style={{ right: '-14px' }}
